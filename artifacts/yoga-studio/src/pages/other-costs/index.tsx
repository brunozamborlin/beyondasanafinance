...
export default function OtherCosts() {
  ...
  const { data: costs, isLoading, refetch } = useListOtherCosts({ month: currentMonth });
  ...
  return (
    <PageTransition className="min-h-screen bg-background">
      ...
      <div className="p-6 space-y-3 pb-8">
        ...
        {costs?.length === 0 && <div className="text-center py-12 text-muted-foreground">Nessuna spesa aggiuntiva questo mese</div>}
      </div>

      {showAdd && <AddCostModal month={currentMonth} onClose={() => setShowAdd(false)} onSuccess={() => refetch()} />}
    </PageTransition>
  );
}
